...
type DetailedStatusClient interface {
	...
	Get(lbServiceIdParam string, lbVirtualServerIdParam string, enforcementPointPathParam *string, sourceParam *string) (nsx_policyModel.AggregateLBVirtualServerStatus, error)
}
...
func NewDetailedStatusClient(connector vapiProtocolClient_.Connector) *detailedStatusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.lb_services.lb_virtual_servers.detailed_status")
...
func (dIface *detailedStatusClient) Get(lbServiceIdParam string, lbVirtualServerIdParam string, enforcementPointPathParam *string, sourceParam *string) (nsx_policyModel.AggregateLBVirtualServerStatus, error) {
	...
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.lb_services.lb_virtual_servers.detailed_status", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.AggregateLBVirtualServerStatus
