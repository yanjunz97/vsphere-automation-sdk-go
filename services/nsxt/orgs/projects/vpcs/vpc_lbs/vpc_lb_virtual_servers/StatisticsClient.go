...
type StatisticsClient interface {
	...
	Get(orgIdParam string, projectIdParam string, vpcIdParam string, vpcLbIdParam string, vpcLbVirtualServerIdParam string, enforcementPointPathParam *string, sourceParam *string) (nsx_policyModel.AggregateLBVirtualServerStatistics, error)
}
...
func NewStatisticsClient(connector vapiProtocolClient_.Connector) *statisticsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.vpcs.vpc_lbs.vpc_lb_virtual_servers.statistics")
...
func (sIface *statisticsClient) Get(orgIdParam string, projectIdParam string, vpcIdParam string, vpcLbIdParam string, vpcLbVirtualServerIdParam string, enforcementPointPathParam *string, sourceParam *string) (nsx_policyModel.AggregateLBVirtualServerStatistics, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.AggregateLBVirtualServerStatistics
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.vpcs.vpc_lbs.vpc_lb_virtual_servers.statistics", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.AggregateLBVirtualServerStatistics
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.AggregateLBVirtualServerStatistics), nil
	} else {
