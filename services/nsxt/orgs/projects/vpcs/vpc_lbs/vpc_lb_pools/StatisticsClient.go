...
type StatisticsClient interface {
	...
	Get(orgIdParam string, projectIdParam string, vpcIdParam string, vpcLbIdParam string, vpcLbPoolIdParam string, enforcementPointPathParam *string, sourceParam *string) (nsx_policyModel.AggregateLBPoolStatistics, error)
}
...
func NewStatisticsClient(connector vapiProtocolClient_.Connector) *statisticsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.vpcs.vpc_lbs.vpc_lb_pools.statistics")
...
func (sIface *statisticsClient) Get(orgIdParam string, projectIdParam string, vpcIdParam string, vpcLbIdParam string, vpcLbPoolIdParam string, enforcementPointPathParam *string, sourceParam *string) (nsx_policyModel.AggregateLBPoolStatistics, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.AggregateLBPoolStatistics
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.vpcs.vpc_lbs.vpc_lb_pools.statistics", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.AggregateLBPoolStatistics
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.AggregateLBPoolStatistics), nil
	} else {
