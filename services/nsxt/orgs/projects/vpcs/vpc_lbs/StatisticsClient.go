...
type StatisticsClient interface {
	...
	Get(orgIdParam string, projectIdParam string, vpcIdParam string, vpcLbIdParam string, enforcementPointPathParam *string, sourceParam *string) (nsx_policyModel.AggregateLBServiceStatistics, error)
}
...
func NewStatisticsClient(connector vapiProtocolClient_.Connector) *statisticsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.vpcs.vpc_lbs.statistics")
...
func (sIface *statisticsClient) Get(orgIdParam string, projectIdParam string, vpcIdParam string, vpcLbIdParam string, enforcementPointPathParam *string, sourceParam *string) (nsx_policyModel.AggregateLBServiceStatistics, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.AggregateLBServiceStatistics
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.vpcs.vpc_lbs.statistics", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.AggregateLBServiceStatistics
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.AggregateLBServiceStatistics), nil
	} else {
