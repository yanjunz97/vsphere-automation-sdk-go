...
type AlarmsClient interface {
	...
	Get(orgIdParam string, projectIdParam string, vpcIdParam string) (nsx_policyModel.PolicyVpcSecurityPoliciesAlarm, error)
}
...
func NewAlarmsClient(connector vapiProtocolClient_.Connector) *alarmsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.vpcs.security_policies.alarms")
...
func (aIface *alarmsClient) Get(orgIdParam string, projectIdParam string, vpcIdParam string) (nsx_policyModel.PolicyVpcSecurityPoliciesAlarm, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.PolicyVpcSecurityPoliciesAlarm
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := aIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.vpcs.security_policies.alarms", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.PolicyVpcSecurityPoliciesAlarm
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.PolicyVpcSecurityPoliciesAlarm), nil
	} else {
