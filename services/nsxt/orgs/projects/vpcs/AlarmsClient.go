...
type AlarmsClient interface {
	...
	// @return com.vmware.nsx_policy.model.PolicyVpcAlarm
	//
	...
	Get(orgIdParam string, projectIdParam string, vpcIdParam string) (nsx_policyModel.PolicyVpcAlarm, error)
}
...
func NewAlarmsClient(connector vapiProtocolClient_.Connector) *alarmsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.vpcs.alarms")
...
func (aIface *alarmsClient) Get(orgIdParam string, projectIdParam string, vpcIdParam string) (nsx_policyModel.PolicyVpcAlarm, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.PolicyVpcAlarm
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := aIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.vpcs.alarms", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.PolicyVpcAlarm
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.PolicyVpcAlarm), nil
	} else {
